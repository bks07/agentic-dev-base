...
- **App Repo Boundary:** Never edit files outside the selected app repo. If the provided scope crosses into another app repo, stop and ask for correction.
...
- Read the selected app's `constitution.md` or equivalent product context first when behavior depends on product intent.
- Inspect repo-local manifests, READMEs, and conventions before assuming commands or structure.
...
- **Performance First:**
    ...
    - **React:** Use `React.memo` and `useCallback` only when they match the app repo's established patterns and compiler guidance.
...
1. Change Summary
    - What was implemented, for which app repo, and why.
2. Files Changed
